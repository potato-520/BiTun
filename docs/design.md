...
系统需具备以下三大特性：
1. **中性角色与对冲打洞**：不依赖特定的一方作为传统的“主动连接发起方”。采用对冲打洞（Hole Punching）技术，一旦任意一端发起 UDP 探测并得到应答，即在底层确立 UDP 传输信道。
...
在两端没有 STUN/TURN 服务器辅助、且可能均处于 NAT/防火墙内侧的情况下，系统采用完全对称的对等端（Peer-to-Peer）对冲打洞与握手状态机设计，以解决 UDP 双向连通性与连接的稳健维持。
...
通过这种对称设计，系统既支持两端都配置对端地址的“双向对冲打洞”（以应对恶劣 of 对称 NAT 环境），也支持一端固定公网 IP 而另一端动态学习的“单向打洞与捕获”模式。

### 4.2 对冲打洞握手状态机 (Symmetric Punching & Handshake State Machine)
...
以下是 Peer A（主动探测）与 Peer B（未配置地址，动态学习）建立连接的对称对冲时序图：
