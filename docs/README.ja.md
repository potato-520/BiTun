...
* *注*：`--odd` および `--even` コマンドライン引数は非推奨となり、無視されます。チャネルID生成の奇数／偶数は、ハンドシェイク時に生成されるランダムなチャレンジソルト（Challenge Salt）の辞書順比較（`R_local` vs `R_remote`）によって自動でネゴシエーションされます（本端のランダムソルトが大きい方が ODD (1)、小さい方が EVEN (0) になります）。これにより、IDの衝突を防ぎます。
...
* **Peer A** (UDP 9000番ポートでSOCKS5プロキシを起動し、Peer Bに向けてホールパンチを実行)：
  ```bash
  ./bitun -p 9000 -r 127.0.0.1:9001 -k MySecretPSKKey123456789012345678
  ```
* **Peer B** (UDP 9001番ポートでSOCKS5プロキシを起動し、Peer Aに向けてホールパンチを実行)：
  ```bash
